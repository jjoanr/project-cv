...
def extract_prototypes(model, dataset, indices, device, batch_size=32):
    ...
    # We use val_transform here so the support images aren't distorted when calculating the final center
...
def main():
    ...
    for cls, idxs in class_indices.items():
        ...
        # We want args.images_per_class for training, but MUST leave at least 2 for validation
        train_count = min(args.images_per_class, total - 2)
        
        # If the class is incredibly tiny, fallback to 80/20, but force at least 2 train images
        if train_count < 2:
    ...
    # Find the smallest pool of images across all classes in both train and val sets
    min_train = min(train_counts)
    min_val = min(val_counts)
    safe_pool = min(min_train, min_val)
    
    # Automatically adjust shots to fit the bottleneck class
    ...
    # 4. FINAL LEAK-FREE EVALUATION & CONFUSION MATRIX
    ...
    # Evaluate strictly on the hold-out validation set
